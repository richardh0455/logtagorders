...
class CreateOrder extends React.Component{
 ...
 createOrUpdate() {
   if(this.state.isUpdate){
     ...
     <div data-field-span="1" >
       <label>Order</label>
       <Select value={this.state.currentlySelectedOrder} onChange={this.handleOrderChange} options={this.state.orders.filter(order => {return !this.fieldHasValidValue(order.ShippedDate)}).map(order => {return {value:order.InvoiceID, label: order.LogtagInvoiceNumber}}) isSearchable="true" placeholder="Select an Order"/>
      </div>
      </div>
